...
exports.requestDelivery = functions.https.onCall(async(data, context) => {
    ...
    console.log("agents.docs.length: ", agents.docs.length);

    for (let index = 0; index < agents.docs.length; index++) {
        const agentDoc = agents.docs[index];
        // const agentData = agentDoc.data();
        // console.log("agentDoc: ", agentDoc);
        // console.log("agentDoc.data(): ", agentDoc.data());

        console.log("agent agentDoc['online']: ", agentDoc.get('online'));
        console.log("agent agentDoc['status']: ", agentDoc.get('status'));
        if (agentDoc.get('connected') == true && agentDoc.get('online') == true && agentDoc.get('status') == "ACTIVE") {
            console.log("agent id: ", agentDoc.id);

            // const missionsInProgress = await firestore.collection("agents").doc(agentDoc.id).collection("orders").where("status", "in", ["DELIVERY_ACCEPTED", "DELIVERY_REQUESTED", "SENDED"]).get();

            console.log("agentDoc['mission_in_progress']", agentDoc.get('mission_in_progress'));

            if (agentDoc.get('mission_in_progress') == null) {
                const objUpdate = {
                ...
                console.log("agent id: ", agentDoc.id);
                console.log("orderDoc.id: ", orderDoc.id);
                console.log("orderDoc.data(): ", orderDoc.data());

                await firestore.collection("agents").doc(agentDoc.id).collection("orders").doc(orderDoc.id).set(orderDoc.data());
...
exports.agentResponse = functions.https.onCall(async(data, context) => {
    ...
    console.log("orderData: ", orderData.customer_id);
    console.log("orderData: ", orderData.seller_id);
    console.log("orderData: ", data.agentId);
    console.log("data.response.status: ", data.response.status);

    if (data.response.status == "DELIVERY_ACCEPTED") {
        await firestore.collection("agents").doc(data.agentId).update({ mission_in_progress: data.orderId });
    }

    if (data.response.status == "DELIVERY_CANCELED") {
        await firestore.collection("agents").doc(data.agentId).update({ mission_in_progress: null });
    }
    ...
    await firestore.collection(`agents/${data.agentId}/orders`).doc(data.orderId).update(data.response);
});
...
exports.concludeOrder = functions.https.onCall(async(data, context) => {
    ...
    console.log(`(token.toUpperCase() != dataToken.toUpperCase()): ${(token.toUpperCase() != dataToken.toUpperCase())}`);
...
exports.newAddress = functions.https.onCall(async(data, context) => {
    ...
    let addresses = await firestore.collection(`${data.collection}/${data.userId}/addresses`).where("status", "==", "ACTIVE").get();
    ...
    await firestore.collection(`${data.collection}/${data.userId}/addresses`).add(data.address)
        .then(async function(result) {
            await result.update({
                id: result.id,
                created_at: now,
                status: "ACTIVE",
                main: addresses.docs.length == 0,
            });
